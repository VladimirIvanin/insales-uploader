      ...
      if (_owner.queueList[0]) {
        ...
        updateteAssets(_owner).then(function () {
          ...
          if (!_queue.task.unlink) {
            if (_owner.assets[_queue.name]) {
              editAsset (_owner, _queue.content.asset, _owner.assets[_queue.name].id, _queue.path).then(function() {
          ...
          if (_queue.task.unlink && _owner.assets[_queue.name]) {
            removeAsset(_owner, _owner.assets[_queue.name].id, _queue.path, _queue.name).then(function() {
              _owner.queueList.splice(0, 1)
              if (_owner.queueList[0]) {
                return _iterationQueue(_owner);
              }else{
