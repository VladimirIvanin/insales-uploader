...
queueManager.createTask('watch', function (param) {
  ...
  return new Promise(function(resolve, reject) {
    updateteAssets(conf, state).then(() => {
      ...
      // upload
      if (_.isEqual(fileInfo.event, 'add')) {
        if (assets[fileInfo.path]) {
      ...
      // edit
      if (_.isEqual(fileInfo.event, 'change')) {
        if (assets[fileInfo.path] && assets[fileInfo.path].id) {
          editAsset (conf, state, fileInfo.content.asset, assets[fileInfo.path].id, fileInfo.path, task).then(()=>{
            resolve();
          }).catch(() =>{
            resolve();
...
export function setQueueAsset(conf, state, fileInfo, _task, debugMode) {
  let task = _task || {};
  let assets = conf.get('assets');
  let options = conf.get('options');
  let param = {
    conf,
    state,
    fileInfo,
  ...
  queueManager.runTask('watch', param, {
    fileInfo: {
      path: fileInfo.path
    }
  })
