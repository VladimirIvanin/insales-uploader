  ...
  return new Promise(function (resolve, reject) {
    ...
    for (var i = 0; i < assets.length; i++) {
      ...
      if (asset['name'].indexOf('/') > -1 || asset['name'].indexOf('@') > -1) {
        console.error(log_error('Недопустимое имя файла:', asset['name']))
      }
...
export function getAssetPath (paths, _type, _name, _ext, task) {
  var _assets = paths.assets;
  var _mediaExtension = paths.mediaExtension;
...
export function patchThemes (_owner, themes) {
  return new Promise(function (resolve, reject) {
    _owner.themes.list = themes;
    _.forEach(themes, function (_theme, index) {
      if (_.toNumber(_theme.id) == _owner.options.theme.id) {
        _theme.type = typeTheme(_theme);
        _owner.themes.current = _theme;
        resolve();
      }
    });
  });
