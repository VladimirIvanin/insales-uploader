...
export function patchOption (_owner, _options) {
  ...
  if (!_owner.options.id) throw new Error('Missing app id');
  if (!_owner.options.token) throw new Error('Missing app token');
  if (!_owner.options.url) throw new Error('Missing app url');
  _owner.options.protocol = (_owner.options.http) ? 'http://' : 'https://';

  const VRegExp = new RegExp(/^http.*:[www]*\/\//);
  const VResult = _options.url.replace(VRegExp, '');
  _owner.options.url = VResult.replace('/', '');
  _owner.options.handle = _.split(_owner.options.url, '.', 1);
