...
export function writeManager (_asset, _uri, dataReponse, action, _owner, callback) {
  ...
  if (dataReponse.asset_url) {
    if (action.method === 'download' && action.rewrite) {
      ...
      if (!action.method === 'release' && _owner.options.startBackup || action.method === 'backup') {
        writeFileWithDownload(_uri, _asset.backupPath).then(function () {
          callback()
        })
      }
      if (action.method === 'release') {
        writeFileWithDownload(_uri, _asset.releasePath).then(function () {
          callback()
        })
      }
    }
    ...
    if (action.method === 'download' && action.rewrite) {
      ...
      if (action.method === 'release') {
        writeFile(_asset.releasePath, _contents, 'utf8').then(function () {
          callback()
        })
      }
    }
