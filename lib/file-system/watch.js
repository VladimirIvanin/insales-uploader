    ...
    fs.readFile(_path, (err, data) => {
      ...
      if (isMediaFile) {
        if (_assetType === 'Asset::Media') {
          var _filePathAsset = getAssetPath(_owner.paths, _assetType, _fileName, _ext, { isAssetsPath: true });
...
export function getFileInfo(_path, _root, _paths, task) {
  return new Promise(function (resolve, reject) {
  var fileInfo = {};
  var _task = task || {};

  var pathNormal = path.normalize(_path);
  var pathMedia = path.normalize(_root+'/media');
  var pathParse = path.parse(pathNormal);
  var _fileName = pathParse.base;
  var _ext = pathParse.ext;
  var _dirPath = pathParse.dir;
  var _folderParent = _.last(_dirPath.split(path.sep));
  var _assetType = _.findKey(_paths.assets, function(o) { return o.folder.indexOf(_folderParent) > -1  }) || 'Asset::Media';

  fileInfo.name = _fileName;
  fileInfo.type = _assetType;
