 ...
 */
export function _watch(conf, state, event, _path) {
  if (/\s/g.test(_path)) {
  ...
  getFileInfo(_path, paths, event).then(fileInfo => {
    ...
    if (_.eq(fileInfo.type, 'Asset::Media')) {
      ...
      if (fileInfo.synchronizePath !== '') {
        syncFile(fileInfo.path, fileInfo.synchronizePath, fileInfo.encode, event)
      }
    }
 ...
 */
function syncFile(file1, file2, encode, event) {
