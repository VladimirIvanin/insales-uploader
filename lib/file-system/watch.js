 ...
 */
export function _watch(conf, state, event, _path, debugMode) {
  if (/\s/g.test(_path)) {
  ...
  getFileInfo(_path, paths, event).then(fileInfo => {
    if (debugMode) {
      ...
      console.log(log_notice('Event: ') + `${event}`)
 ...
 */
function syncFile(file1, file2, encode, event, debugMode) {
...
export function watcher(conf, state) {
  ...
  chokidar.watch(paths.toWatch, _options).on('all', (event, _path) => {
    _watch(conf, state, event, _path, options.tools.debugMode);
  });
}
...
export function getAssetInfo(_path, paths) {
  ...
  let _assetType = _.findKey(paths.assets, function(o) { return o.folder.indexOf(_folderParent) > -1  }) || 'Asset::Media';
