...
import { writeFile,
         writeFileList } from '../file-system/file';
import { getFileInfo,
...
export function initAssets(paths) {
  ...
  const media_root = upath.normalize(paths.folders.media_root);
  const files = fs.readdirSync(media_root);
    ...
    _.forEach(files ,function (fileName, index) {
      var pathNormal = upath.normalize(media_root + fileName);
      ...
      fs.readFile(pathNormal, (err, data) => {
        ...
        listTask.push(_result)

        fileCount++
        if (fileCount == _.size(files)) {
          writeFileList(listTask).then(() => {
            resolve();
          });
        }
