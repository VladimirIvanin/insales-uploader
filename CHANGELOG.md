# Change Log

## [0.27.5]

### Исправлено

— Скачивание файлов на кириллице.

---

## [0.27.4]

### Исправлено

— Добавлен промис для метода `pushTheme`.

---

## [0.27.3]

### Добавлено

— Возможность добавления postcss плагинов.

— Метод start (download => stream).

— Настройка для домена ассетов.

— Проверка сети (onLine/Offline)

### Исправлено

— Ошибки при парсинге es6 синтаксиса.

## [0.27.2]

— Исправлена ссылка на ассеты.

## [0.27.1]

— Исправлена ошибка при сохранении css/js файлов включающих в себя #=.

## [0.27.0]

— Обновлены npm пакеты.

— Передача настроек в модуль chokidar (options.tools.chokidar).


## [0.26.0]

— Добавлены метки времени.

## [0.25.0]

— Добавлен ESLint.

## [0.24.0]

— Добавлен autoprefixer.

## [0.23.0]

— Добавлен stylelint.

## [0.22.0]

— Добавлены исключения для отслеживания файлов.

— Добавлена системная нотификация.

## [0.21.12]

— Исправления проблем в работе метода stream.

## [0.21.6]

— Оптимизация метода download.

## [0.21.4]

— Добавлен debug mode, работает на методе stream.

— Изменен порядок скачивания файлов темы.

## [0.21.1]

— Исправлено распределение ассетов.

— Добавлена ссылка на список изменений.

## [0.21.0]

— setTimeout <=> delay.

— Поддержка абсолютных путей.

— Связь модулей с помощью configstore.

— Обновление архитектуры.

## [0.20.5]

— Добавлено уведомление о новых версиях.

## [0.20.4]

— Обновление модуля для скачивания файлов по http.

## [0.20.3]

— Исправление ошибки при записи файлов.

— Исправление метода pullTheme.

## [0.20.1]

— Фиксирование ошибок при запуске стрима.

## [0.20.0]

— Обновление пакетов.

## [0.19.39]

— Исправлены недостатки при скачивании файлов.

## [0.19.38]

— Исправление удаления ассета из assets.

## [0.19.37]

— Исправление копирования ассета из media в assets.

## [0.19.36]

— Исправление загрузки темы через pushTheme.

— Исправление скачивания темы через pullTheme.

## [0.19.25]

— Добавлен метод pullTheme.

## [0.19.23]

— Добавлен метод pushTheme.

## [0.19.20]

— Добавлен progress-bar на метод download.

— Добавлен debounce для BrowserSync.


## [0.19.18]

— Добавлена проверка на спецсимвол \x03.

## [0.19.17]

— Обработка 404 статуса.

## [0.19.16]

— Добавлена цветовая схема.

## [0.19.15]

— Исправления метода updateListThemes.

## [0.19.13]

— Исправления метода stream.

## [0.19.12]

— Добавлен метод triggerFile.

## [0.19.7]

— Исправления метода stream.

## [0.19.6]

— Обновление настроек Browsersync.

## [0.19.4]

— Обновление описания, добавлен таймаут на обновление браузера.

## [0.19.3]

— Добавлен метод upload.

## [0.19.2]

— Добавлен Browsersync для синхронизации сайта и изменений в файлах.

## [0.19.1]

— Добавлено открытие браузера при старте стрима.

## [0.19.0]

— Добавлен метод updateListThemes, добавлено уведомление о статусе темы.

## [0.18.11]

— Добавлено уведомление об отсутсвующих файлах в локальной версии темы.

## [0.18.10]

— Добавлена проверка на разницу файлов перед запуском стрима.

## [0.18.9]

— Добавлен метод diffLocalAssets.

## [0.18.7]

— Бэкапы хранятся только в zip.

## [0.18.6]

— Оптимизация отслеживания изменений в файлах.

## [0.18.4]

— Оптимизация дублирующихся запросов.

## [0.18.3]

— Исправление конфликта сниппетов и шаблонов с одинаковым именем.

## [0.18.2]

— Добавлен метод для сортировки аcсетов из папки media.

## [0.18.0]

— Добавлена синхронизация папок 'media' и 'assets'.

## [0.17.6]

— Правки нейминга свойств.

## [0.17.5]

— Исправление создания директорий.

## [0.17.4]

— Добавлена настройка «assets», при значении false работа с медиа файлами ведется через папку media.

— Исправлена обработка ошибок при watch.

## [0.17.2]

— Исправлена дата в названии архива бэкапа.

— Добавлен вывод ссылки на превью темы в консоль в методе download.


## [0.17.1]

— Оптимизация отправки изменений на сервер.

## [0.17.0]

— Новый способ записи настроек.

## [0.16.0]

— Удален метод release.

— Backup сохраняется в zip, каталоги внутри папки backup перезаписываются с сервера.

— Добавлен метод backupToZip.

## [0.15.1]

— Добавлен метод release.

— Переименована папка для шаблонов template -> templates.

## [0.14.3]

— Увеличена скорость загрузки темы.

## [0.14.2]

**Fixed bugs:**

— Исправление обновления ассетов.

## [0.14.1]

**Fixed bugs:**

— Добавлено уведомление об ошибки при скачивании файла.

— Исправление нейминга метода.

## [0.13.1]

**Fixed bugs:**

— Исправлено сохранение файлов.

## [0.13.0]

— Повышение скорости загрузки редактируемых файлов.

## [0.12.3]

**Fixed bugs:**

— Исправлена загрузка для MacOs.

## [0.12.2]

**Fixed bugs:**

— Исправлено поведение при правки сниппетов и шаблонов с одинаковым именем.

## [0.12.1]

**Fixed bugs:**

— Исправлено уведомление о некорректных файлах.

— Исправлено создание директорий.

## [0.11.0]

**Fixed bugs:**

— Исправлено поведение методов download, backup, stream.


## [0.10.0]

**Fixed bugs:**

— исправлено поведение метода **backup**.

— оптимизированно удаление и добавление файлов.
